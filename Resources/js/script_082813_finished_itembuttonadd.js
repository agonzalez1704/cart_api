...
function makeItemsDraggable () {
	...
	/*end of cart.droppable function */
} 
...
$(document).on('click', '.cart_item #close', function(event){ eliminateItem(this);});
function isPosibleToAdd (element) {var posible = false;var available = parseInt($('.lblAvailability', element).html());if(available > 0){(available == 1) ? element.remove() : null;--available;
$('.lblAvailability', element).html(available);posible = true;}return posible;}
function checkItemExistence (draggedElement) {var exists = false;/*Check each of the elements*/ $('.items > li').each(function(){if($('.item_id', this).html() == $('.item_id', draggedElement).html()){
exists = true; $('.lblCount', this).html( parseInt($('.lblCount', this).html()) + 1 );}});return exists;}
function increaseItemsInCart() {item_count = $("#item_count");droppedCount = parseInt(item_count.text());/*Update the count of the items in the cart*/item_count.text(++droppedCount);}
function eliminateItem (element) {var clicked_element = $(element).parent();if ($('.lblCount', clicked_element).html() > 1){var count = parseInt($('.lblCount', clicked_element).html());
--count;$('.lblCount', clicked_element).html(count);}else {$(clicked_element).remove(); }$("#item_count").text(parseInt($("#item_count").text()) - 1);returnToItems(clicked_element);}
...
makeItemsDraggable();}
function addToItems (item) { $('.catalog_placeholder').append('<div class="catalog_item"><img class="item_image" src="'+$('.item_image', item).attr('src')+'" alt="Insert image here" /><ul><li>Name: <label class="lblName">'+
...
$('.lblPrice', item).html()+'</label></li><li>Item ID: <label class="item_id">'+$('.item_id', item).html()+'</label></li><li><a class="add_to_cart" href="#">Add to Cart</a></li></ul></div>');}

function addToCart (dragging) {
var totalPrice = $('.lblPrice', dragging).html();
alert(totalPrice);
