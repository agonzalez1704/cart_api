...
function checkItemExistence (draggedElement) {
var exists = false;/*Check each of the elements*/ 
$('.items > li').each(function(){if($('.item_id', this).html() == $('.item_id', draggedElement).html()){exists = true; $('.lblCount', this).html( parseInt($('.lblCount', this).html()) + 1 );sumPrices(draggedElement, 'sum');}});return exists;}
function increaseItemsInCart() {item_count = $("#item_count > label");droppedCount = parseInt(item_count.text());/*Update the count of the items in the cart*/item_count.text(++droppedCount);}
...
sumPrices(dragging, 'sum');}

function sumPrices (dragging, operation) {
	
	var totalPrice = $('.lblPrice', dragging).html();
	totalPrice = totalPrice.substring(2, totalPrice.length - 4);
	var current = parseInt($('#cart_total > label').html());
	switch(operation){
		case 'sum':
			current += parseInt(totalPrice);
			break;
		case 'substract':
			current -= parseInt(totalPrice);
			break;
	}
	$('#cart_total > label').html(current+' ');
}
